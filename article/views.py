...
from django.template import Context #class for variables to template
...
def article(request, article_id=1, comment_page=1):
    ...
    current_page = Paginator(Comments.objects.filter(comments_article_id=article_id), 3)
    args = {}
    args.update(csrf(request))
    args['article'] = Article.objects.get(id=article_id)
    #args['comments'] = Comments.objects.filter(comments_article_id=article_id)
    args['comments'] = current_page.page(comment_page)
...
def addcomment(request, article_id):
    if request.POST and "pause" not in request.session:
        ...
        if form.is_valid():
            ...
            comment.comment_author_id = auth.get_user(request).id
            form.save()
